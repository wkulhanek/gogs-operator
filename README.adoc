...
. Deploy the `gogs-admin-rules` Cluster Role (which will be granted automatically to everyone with `admin` privileges):
+
[source,sh]
----
...
In case you wish to uninstall the Gogs Operator make sure that there are no more projects using the gogs-operator. Then as *cluster-admin* delete the ClusterRole and Custom Resource:
